...
% fourth order in space. 
 ...
 mg=4;   % factor for defining the number of Grid points
...
utotal=zeros(NX+1,1);
  ...
  for i=3:NX-2
     ...
     A_con(1:3,1:5,i) = (1/dt2)*[0, 0, rho(i), 0, 0; 0, 0, -2*rho(i), 0, 0; 0, 0, rho(i), 0, 0];
     
     K_con (1:3,1:5,i)= (1/(24*dt2))*[0 ,0,0,0,0;...
     ...
     A_opt(1:3,1:5,i)= (rho(i)/(90*dt2))*[-1 4 84 4 -1 ; 2 -8 -168 -8 2 ; -1 4 84 4 -1];
     
    
     K_opt(1:3,1:5,i)=(1/(144*dt2))*[-(mu(i-2,1)+mu(i,1)), 16*(mu(i-1,1)+mu(i,1)), -16*(mu(i-1,1)+2*mu(i,1)+mu(i+1,1)),16*(mu(i,1)+mu(i+1,1)), -(mu(i,1)+mu(i+2,1))+(mu(i-2,1)+2*mu(i,1)+mu(i+2,1));...
    ...
    dA(1:3,1:5,i)= A_opt(1:3,1:5,i)-A_con(1:3,1:5,i);
   
    dK(1:3,1:5,i)= K_opt(1:3,1:5,i)-K_con(1:3,1:5,i);
  ...
  for it = 1:NSTEP
    ...
    %   i) Calcul de f=(A0-K0)*u0 (definition de la source secondaire)
        UI(1:3,1:5,i) = [u(i-2) u(i-1) u(i) u(i+1) u(i+2) ; unm1(i-2) unm1(i-1) unm1(i) unm1(i+1) unm1(i+2);unm2(i-2) unm2(i-1) unm2(i) unm2(i+1) unm2(i+2)];
...
%         unm1(i)=u(i);
    ...
    for i=3:(NX-2)
        ...
        u1(i) = 2.d0*u1nm1(i) - u1nm2(i) + C_ijkl(i) * value_du_dxx1 * (DELTAT^2.d0)/rho(i)+g(i);
        % Donne la valeur du coefficient u(i, t+DELTAt) en fonction des
        % coefficients u(i,t) et u(i,t-DELTAT)
    end
