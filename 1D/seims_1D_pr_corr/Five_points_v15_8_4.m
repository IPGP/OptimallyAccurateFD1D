...
%--------------------------------------------------------------------------
  a = pi*pi*F0*F0;
  umaxv = Ampl*exp(-a*T0^2)/2.d0*DELTAT;
  dx2 = DELTAX^2.d0;
  dt2 = DELTAT^2.d0;
  ...
  for i=3:NX
    A_con4(1:3,1:5,i-2) = (rho(i)/dt2)*[0 0 1 0 0; 0 0 -2 0 0;0 0 1 0 0];
    K_con4(1:3,1:5,i-2) = (mu(i)/(12*dx2))*[0 0 0 0 0 ; -1 16 -30 16 -1 ; 0 0 0 0 0];
    A_opt4(1:3,1:5,i-2) = (rho(i)/(90*dt2))*[-1 4 84 4 -1 ; 2 -8 -168 -8 2 ; -1 4 84 4 -1];
    K_opt4(1:3,1:5,i-2) = (mu(i)/(144*dx2))*[-1 16 -30 16 -1; -10 160 -300 160 -10;-1 16 -30 16 -1];
    dA4(1:3,1:5,i-2) = (rho(i)/(90*dt2))*[-1 4 -6 4 1; 2 -8 12 -8 2 ;-1 4 -6 4 1];
    dK4(1:3,1:5,i-2) = (mu(i)/(144*dx2))*[-1 16 -30 16 -1; 2 -32 60 -32 2;-1 16 -30 16 -1];
  end  
  ...
  for it = 1:NSTEP
    ...
    %   i) Calcul de f=(A0-K0)*u0 (definition de la source secondaire)
        UI4(1:3,1:5,i-2) = [u(i-2) u(i-1) u(i) u(i+1) u(i+2) ; unm1(i-2) unm1(i-1) unm1(i) unm1(i+1) unm1(i+2);unm2(i-2) unm2(i-1) unm2(i) unm2(i+1) unm2(i+2)];
        %UI=[u(i-1) u(i) u(i+1) ;unm1(i-1) unm1(i) unm1(i+1);unm2(i-1) unm2(i) unm2(i+1)];
    
        g(i)=-sum(sum((dA4(1:3,1:5,i-1)-dK4(1:3,1:5,i-1)).*UI4(1:3,1:5,i-2)))*dt2/rho(i);
