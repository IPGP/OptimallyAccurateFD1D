% General code for 1D homogeneus elastic case without intern boundaries
...
choice = menu('Please choose modelisations method','CONV 2','CONV 4', 'OPT 2', 'OPT 4') ;
...
rho = set(4) ;
lambda = rho * (cp^2 - 2.d0 * cs^2) ; % Lamé's first parameter [u.s.i.]
mu = rho * cs^2 ; % Lamé's second parameter [u.s.i.]
...
%--------------------------------------------------------------------------
      elseif OPT2
          ...
          for i = 2:NX
              ...
              g(i) = -sum(sum((dA2(1:3,1:3,i-1)-dK2(1:3,1:3,i-1)).*UI2(1:3,1:3,i-1)))*dt2/rho ;
          end
          
          for i = 2:NX
              value_du_dxx = (u1nm1(i-1) - 2*u1nm1(i) + u1nm1(i+1)) / dx2 ;
              u1(i) = 2.d0*u1nm1(i) - u1nm2(i) + (mu * value_du_dxx * dt2)/rho + g(i) ;
          end
