...
%--------------------------------------------------------------------------
  ...
  for it = 1:NSTEP  
    ...
    for i=3:(NX-1)
        ...
        u(i) = 2.d0*unm1(i) - unm2(i) + C(i,1,1,1,1) * value_du_dxx * dt2/rho(i);
        
        v(i) = (u(i) - unm2(i))/(2*DELTAT);
        value_du_dx = (unm1(i-2)-8*unm1(i-1)+8*unm1(i+1)-unm1(i+2)) / (12.d0*DELTAX);
...
%     fprintf('it=%d \n umax=%e\n imax=%d\n\n',it,max(u),find(u==max(u)));
    ...
    i = ISOURCE;
    %u(i) = u(i) + source_term;
    u(i) =u(i) + source_term*DELTAT/rho(i);
