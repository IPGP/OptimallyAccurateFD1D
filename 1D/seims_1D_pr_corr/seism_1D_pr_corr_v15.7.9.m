...
%--------------------------------------------------------------------------
  ...
  for i=2:NX
    A_con(1:3,1:3,i-1) = (rho(i)/DELTAT^2)*[0 1 0; 0 -2 0; 0 1 0];
    K_con(1:3,1:3,i-1) = (mu(i)/DELTAX^2)*[0 0 0; 1 -2 1; 0 0 0];
    A_opt(1:3,1:3,i-1) = (rho(i)/DELTAT^2)*[1/12 10/12 1/12; -2/12 -20/12 -2/12; 1/12 10/12 1/12];
    K_opt(1:3,1:3,i-1) = (mu(i)/DELTAX^2)*[1/12 -2/12 1/12; 10/12 -20/12 10/12; 1/12 -2/12 1/12];
  ...
  for it = 1:NSTEP
    ...
    %   On ne garde que la colonne du milieu de fs que l on assigne a fs2
    %   (vecteur colonne de NX elements)
    %   fs2=fs(:,2);
    ...
    for i=2:NX
        ...
        u1(i) = 2.d0*u1nm1(i) - u1nm2(i) + C(i,1,1,1,1) * value_du_dxx * (DELTAT^2.d0)/rho(i)+g(i);
        % Donne la valeur du coefficient u(i, t+DELTAt) en fonction des
        % coefficients u(i,t) et u(i,t-DELTAT)
    end
...
%   ii) Calcul de f1=-(dA-dK)*u0    
%         for k=(-1):(1)
%             
%     %       On modifie chacun des termes de UI pour obtenir les coefficients
%     %       d une nouvelle matrice f1s
%             u1(i)=-(dA(1,2+k)-dK(1,2+k)).*u(i+k);       % correspond a 3 scalaires
%             u1nm1(i)=-(dA(2,2+k)-dK(2,2+k)).*unm1(i+k); % correspond a 3 scalaires
%             u1nm2(i)=-(dA(3,2+k)-dK(3,2+k)).*unm2(i+k); % correspond a 3 scalaires
%             
%             
%             f1s=[u1(i-1) u1(i) u1(i+1) ;u1nm1(i-1) u1nm1(i) u1nm1(i+1);u1nm2(i-1) u1nm2(i) u1nm2(i+1)];
%             
%             g1(i)=u1(i);
%         
%     %       On ne garde que la colonne du milieu de f1s que l on assigne a f1s2 
%     %       (vecteur colonne de NX elements)
%     %       f1s2=f1s(:,2);
%         
%         end
    
        % On fait en sorte que les coefficients corriges soient utilises
        % pour l iteration suivante

    

            
    %   iii) Calcul de g modifie
    
%         fs_corr=svdinv(A_con-K_con).*(fs+f1s);
%         
%         g(i)=fs_corr(1,2);
% 
% 
...
%     fprintf('it=%d \n umax=%e\n imax=%d\n\n',it,max(u),find(u==max(u)));
    ...
    u=u+u1; % sommer des u0 et u1 pour ce time step
    % shift des u0
    unm2=unm1;
    unm1=u;
